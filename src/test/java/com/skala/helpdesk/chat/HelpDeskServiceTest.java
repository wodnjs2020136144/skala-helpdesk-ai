...
/**
 * 담당: A. Phase 5(교안 p.319) — 실제 OpenAI 호출 없이 conversationId 규칙·출처 추출·
 * SafeGuard-메모리 순서 계약을 검증한다. 실제 pgvector·OpenAI를 쓰는 통합 검증(멀티턴 맥락
 * 유지, 세션·학번 격리, 재기동 후 유지, 20개 윈도우 트리밍)은 이 테스트로 대체되지 않는다
 * — {@code docs/검증-시나리오.md}에 실측 결과를 별도로 기록한다.
 */
class HelpDeskServiceTest {
    ...
    void conversationId는_테넌트_학번_세션을_합쳐_규칙대로_만든다() {
        HelpDeskService service = new HelpDeskService(null, null);
    ...
    void SafeGuard가_차단한_입력은_메모리에_저장되지_않는다() {
        ...
        HelpDeskService service = serviceWithChain(saved);
    ...
    private static HelpDeskService serviceWithChain(List<Message> saved) {
        ChatMemoryRepository repository = recordingRepository(saved);
        ChatMemory chatMemory = MessageWindowChatMemory.builder()
                .chatMemoryRepository(repository)
                .maxMessages(20)
                .build();
        ChatClient chat = ChatClient.builder(new ScriptedChatModel())
                .defaultAdvisors(
                        new SafeGuardAdvisor(),
                        MessageChatMemoryAdvisor.builder(chatMemory).order(200).build())
                .build();
        return new HelpDeskService(chat, chatMemory);
    }
