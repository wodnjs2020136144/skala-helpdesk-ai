 ...
 * {@code Rag.inspectionMaxTopK}는 PR #2의 A 리뷰에 따라 운영 검색과 관리자 진단 검색의
 * 범위를 분리하기 위해 추가했다. 진단 상한도 코드 상수가 아니라 설정으로 관리한다.
 *
 * @param rag    RAG 검색·인제스트 파라미터(top-k·진단용 top-k 상한·유사도 임계값·청크 크기 —
 *               day3-consult-agent 트러블슈팅 참고, text-embedding-3-small 기준 실측치로 조정한다)
...
public record HelpDeskProperties(Rag rag, Memory memory, Tool tool) {
    ...
    public record Rag(int topK, int inspectionMaxTopK, double threshold,
                      int chunkSize, int minChunkSizeChars) {}
