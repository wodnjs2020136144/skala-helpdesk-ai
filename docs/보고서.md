...
📸 **캡처 17** — `docker compose ps`의 pgvector `Up` 상태와
...
📸 **캡처 18** — 관리자 인증으로 실행한 `threshold=0&topK=20` 응답에서 점수순 청크와
...
📸 **캡처 19** — 출처가 포함된 정상 HTTP 200 응답과 빈 질문의 HTTP 400·필드 오류 응답을
나란히 비교한다.
...
📸 **캡처 20** — “알고리즘 수강철회” 요청에 `WD-0001/PENDING`이 반환되고 승인 대기
목록에 남아 있는 화면을 제시한다.

📸 **캡처 21** — `ToolSchemaContractTest`에서 노출 Tool이 세 개뿐이며 입력 스키마에
...
📸 **캡처 22** — 같은 `sessionId=s1`을 사용한 2021001·2021002의 이력이 서로 다른
...
📸 **캡처 23** — 졸업·장학 질문을 동시에 구독해 각각의 token과 source가 다른 요청에
...
📸 **캡처 24** — 같은 관리자 엔드포인트를 미인증·학생·관리자로 호출한 401·403·200
결과와 각 응답의 서버 생성 `X-Trace-Id`를 비교한다.

📸 **캡처 25** — `AI_TOOL_AUDIT`의 채팅·Tool 성공/실패 로그에서 학번이 마스킹되고
...
📸 **캡처 26** — `results.md`의 20/20 결과와 다섯 품질 지표, P95, 호출 오류 0건을
한 화면에 제시한다.
