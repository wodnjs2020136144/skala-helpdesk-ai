...
**SKALA University 학사 안내 상담 에이전트**다. 학생이 자연어로 물으면 학사규정을 근거로
...
| Phase | 요구사항 | 담당 | 구현 | 검증 |
|---|---|:---:|---|---|
| 1 | 모델·pgvector 설정, ChatClient 조립, Advisor 순서 | A | `AiConfig` · `HelpDeskProperties` · `application.yml` | 기동·Health(캡처 3) |
| 1 | 실행 환경, Health, Swagger, 감사 Advisor 골격 | B | `docker-compose.yml` · `AuditAdvisor` | 캡처 17 |
| 2 | 문서 인제스트·청크·메타데이터·재색인 | A | `IngestService` — md 3종 + 규정 PDF 3종 | 청크 306건(캡처 4) |
| 2심화 | 관리자 청크 검사 API | B | `AdminController#chunks` | 캡처 7·18 |
| 3 | RAG 답변 생성·출처 반환·근거 없음 처리 | A | `HelpDeskService#sourcesFrom` | 시나리오 ①(캡처 6) |
| 3 | `AnswerDto` 계약·입력 검증·Swagger 예시 | B | `AnswerDto` · `ChatController` | `ChatControllerTest` 6개 |
| 4 | 학적 조회·수강철회 Tool, 소유자 검증 | B | `AcademicTools` · `RequestTools` | 시나리오 ②④(캡처 20) |
| 5 | JDBC 대화 메모리, 20개 윈도우, 격리 | A | `ChatMemoryConfig` · `conversationId` | 시나리오 ③(캡처 8) |
| 5 | 3턴 시나리오·격리·재기동 유지 테스트 | B | `HelpDeskServiceMemoryTest` 5개 | 캡처 22 |
| 6 | 스트리밍 서비스 로직, 요청 단위 출처 | A | `HelpDeskService#streamEvents` | 캡처 10 |
| 6 | SSE Controller, `token`·`sources`·`done` | B | `ChatController#stream` | `ChatStreamControllerTest` 6개 |
| 7 | SafeGuard, 인젝션 차단, 메모리 미저장 | A | `SafeGuardAdvisor` · `RetrievalGuard` | 시나리오 ⑥(캡처 12·13) |
| 7 | 인증·인가, 승인 게이트, 감사 로그, 레드팀 10종 | B | `SecurityConfig` · `TraceIdFilter` · `ToolAuditAspect` | 캡처 24·25 |
| 8 | 토큰·지연 계측, 모델 폴백, 비용 상한 | A | `TokenMeterAdvisor` · `BoundedToolCallingManager` | 캡처 14·15·16 |
| 8 | Golden Set, P95, Tool 성공률, 장애 주입 | B | `GoldenSet` · `goldenSetTest` | 20/20(캡처 26) |
...
특히 **실제 규정 PDF를 쓸 수 있다**는 점이 컸다. 예제용 짧은 문서로는 검색 실패도,
조항이 뒤섞이는 문제도 관찰되지 않는다. 3장 3.2에서 보듯 실제 규정을 넣고 나서야
"부칙이 본칙보다 위에 검색된다" 같은 문제가 드러났다.

### 1.4 팀 구성과 분업 원칙

2인 1조. **황재원(A) — AI·RAG·플랫폼 / 박성우(B) — 업무 API·Tool·보안.**
...
| 파일 | 최종 소유자 | 상대 역할 |
|---|---|---|
| `AiConfig` · `application.yml` · `HelpDeskService` | A | B가 보안·권한 관점 검토 |
| `AnswerDto` · `ChatController` · `SecurityConfig` | B | A가 RAG 출처·ChatClient 연결 검토 |
...
이 프로젝트에서 가장 중요한 설계 결정이다. **순서를 바꾸면 기능이 아니라 정책이 바뀐다.**
...
A가 조립하고 B가 보안 관점에서 검토한 **공동 결정 사항**이다.
...
꺼낼 수 없게 만드는 것은 다르다. **뚫리면 프롬프트가 아니라 코드로 막는다**가 이 프로젝트의
규칙이다.
...
📸 **캡처 19** — 정상 응답의 `answer`·`sources`·`toolUsed` 계약

![캡처 19](보고서-이미지/03-시나리오123.png)
...
📸 **캡처 21** — 승인은 관리자 목록에만 있고 Tool에는 없다

![캡처 21](보고서-이미지/04-시나리오4-승인게이트.png)
...
📸 **캡처 22** — 같은 질문이라도 인증 주체별로 다른 학점만 반환된다

![캡처 22](보고서-이미지/05-시나리오5-소유자검증.png)
...
📸 **캡처 23** — SSE 이벤트 순서와 요청 단위 출처

![캡처 23](보고서-이미지/08-sse.png)
...
3·4장이 "무엇을 어떻게 만들었나"라면 이 장은 **"그게 실제로 동작하는가"**다. 아래 캡처는
전부 이 보고서를 쓰면서 다시 실행한 결과다.
...
**⑤가 이 실습에서 가장 중요한 경계**다. 인증 없이 부르면 401, `-u 2021002:student`로
...
**④는 "하지 않는 것"을 보이는 시나리오**다. `WD-0001`/`PENDING`으로 접수만 되고,
학생이 승인 API를 직접 부르면 403이다.

![시나리오 4](보고서-이미지/04-시나리오4-승인게이트.png)

**⑦은 설정을 실제로 망가뜨려 확인**했다. 주 모델을 존재하지 않는 값으로 바꿔 재기동해도
...
**핵심 사실 · 기대 출처 · Tool 사용 · 근거 없음 정책**만 검사한다.
...
**실패를 숨기지 않은 것이 오히려 가장 큰 성과였다.** 75%를 100%로 만드는 과정에서
찾은 것이 부칙 혼입·환각·출처 소실이고, 이 셋이 7장 차별화 포인트가 됐다.
...
📸 **캡처 29** — `./gradlew clean build` — 135개 통과

![캡처 29](보고서-이미지/15-빌드.png)
...
**리뷰가 정확히 반반**이라는 점이 중요하다. 한쪽이 일방적으로 검사하는 구조가 아니라
**양방향**이었다는 뜻이다. 실제로 아래 6.2의 결함도 양쪽에서 고르게 나왔다.

### 6.2 리뷰가 실제로 잡아낸 결함

이 절이 **2인 1조로 한 값어치**를 보여준다. 전부 "혼자였으면 그냥 통과했을" 것들이다.

**① 도구 호출 상한을 진입 횟수가 아니라 배치 크기로 세야 한다** `[B → A]`
`DefaultToolCallingManager.executeToolCalls()`는 한 번의 호출 안에서 도구를 **for 루프로
전부 실행**한다. 진입 횟수를 세면 한 턴에 도구 5개를 부르는 경우가 1회로 계산돼 상한이
헐거워진다.
→ *구현자는 "상한이 걸린다"까지만 확인했다. 라이브러리 내부 루프까지 열어 본 건 리뷰어였다.*

**② Golden Set `AC-01`의 기대값이 부칙 기준이었다** `[A → B]`
"최대 23학점"을 정답으로 고정하고 있었는데, 23학점은 **1999학년도 이전 입학자 특례**다.
현행 본칙은 21학점(+성적우수자 3학점).
→ *테스트가 초록불이어서 아무도 의심하지 않았다. 규정 원문을 대조한 리뷰에서 드러났다.*

**③ 폴백 최종 실패 경로에만 메모리 복구가 빠져 있었다** `[B → A]`
세 실패 경로 중 둘은 복구했는데 "폴백까지 실패" 경로만 누락됐다. 그대로 두면 답변 없는
질문이 남아 다음 턴을 오염시킨다.
→ *정상·1차 실패는 테스트했지만 이중 실패는 아무도 안 돌려 봤다.*

**④ SSE `sources`가 `"[]"` 하드코딩이라 동기 API와 불일치했다** `[A → B]`
스트리밍은 화면상 잘 동작하는 것처럼 보였다.
→ *"토큰이 흐른다"와 "계약이 지켜진다"는 다르다.*

**⑤ `AuditAdvisor`가 `conversationId` 포맷을 직접 파싱하고 있었다** `[A → B]`
`split(":", 3)`으로 학번을 꺼내 쓰고 있었다. 포맷을 바꾸면 조용히 깨지는 암묵적 의존이다.
→ 공개 헬퍼 `studentIdFrom()`으로 정리했다.

**⑥ 운영 검색과 관리자 진단의 top-k 범위를 분리해야 한다** `[A → B]`
진단은 넓게 봐야 하고 운영은 좁아야 하는데 같은 값을 쓰고 있었다.
→ `inspection-max-top-k`를 분리했다.

**⑦ `doFinally`가 다운스트림 전파 뒤에 실행되는 경합** `[B → A]`
...
2. 부칙을 걸러내자 이번엔 "18학점"이 나왔다. 정답 청크를 열어 보니 Tika가 뽑은
   **페이지 머리말·꼬리말이 조문 한가운데를 관통**해 임베딩이 흐려져 있었다.
...
카운터로 상한을 건다. 교차 리뷰 지적을 받아 **진입 횟수가 아니라 배치 크기**를 누적하도록
...
- 아직 재현하지 못한 항목은 **`☐`로 남기고 이유를 적었다.** 예: SSE 오류·타임아웃 경로는
  한동안 `☐`였고, 나중에 실측하면서 **스트림 실패 시 메모리가 복구되지 않는 결함**을 찾았다
...
| 포인트 | 담당 |
|---|---|
| 실제 학사규정 PDF 3종 인제스트(Tika) — 교안은 md 앵커만 | A |
| 스트림 실패·취소 시 대화 메모리 복구 | A |
| 모델 폴백 + 토큰 예산 경고 + Micrometer 지표 | A |
| Golden Set 자동 평가를 일반 빌드와 분리(`goldenSetTest`) | B |
| 관리자 청크 진단 API — 운영 검색과 진단 범위 분리 | B |
| `TraceIdFilter` + 감사 로그 학번 마스킹 + 예외 메시지 비노출 | B |
| 승인 게이트를 Tool 목록에서 제외한 구조 | B |
...
가장 크게 배운 것은 **RAG의 품질이 모델이 아니라 인제스트에서 결정된다**는 점이다.
...
가장 어려웠던 것은 **"동작한다"와 "계약이 지켜진다"를 구분하는 일**이었다. 스트리밍은
...
점을 놓쳐 리뷰에서 지적받았다. 리액티브에서는 "언제 실행되는가"가 "무엇을 하는가"만큼
중요하다는 걸 몸으로 배웠다.
...
이득이 적다"며 미뤄 뒀는데, 막상 실측하니 그 안에 결함이 숨어 있었다. **측정을 미룬
자리가 결함이 숨는 자리였다.**
...
이 보고서의 성격에 맞게, **아직 못 한 것과 못 막는 것**을 그대로 적는다.
