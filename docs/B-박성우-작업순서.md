...
> 선행조건: A의 `HelpDeskService#streamEvents`와 요청 단위 출처 전달 방식이 결정되어야 한다.
...
- [x] 스트리밍 종료 시 해당 요청의 출처를 어떻게 전달할지 정한다.
- [x] 싱글턴 필드 `lastSources`는 사용하지 않는다.
- [x] 동시 요청에서도 출처가 섞이지 않는 요청 단위 구조를 사용한다.

### 작업

- [x] `token` 이벤트로 모델의 텍스트 조각을 전달한다.
- [x] 스트림 마지막에 실제 `sources` 이벤트를 전달한다.
- [x] 정상 종료를 알리는 `done` 이벤트를 추가한다.
- [x] 설정의 60초 타임아웃을 유지한다.
- [x] 사용자가 연결을 끊으면 모델 스트림도 취소되는지 확인한다.
- [x] 스트리밍 오류 응답 방식을 정한다.
- [x] 같은 시각에 두 요청을 보내 출처·세션이 섞이지 않는지 확인한다.
- [x] 동기 API와 SSE가 동일한 사용자·세션 식별 규칙을 사용한다.

### 완료 기준

- [x] SSE가 토큰을 순차적으로 전달한다.
- [x] 마지막 출처가 빈 자리표시자 `[]`가 아니라 실제 검색 출처다.
- [x] 동시 요청·취소·타임아웃 테스트를 통과한다.
- [ ] A가 ChatClient·RAG 연결 관점에서 리뷰한다.

### 2026-08-22 구현·검증 결과

- PR #9의 요청 단위 `StreamEvent`를 `token* → sources → done` SSE로 직렬화한다.
- 모델 오류와 설정 타임아웃은 내부 예외를 숨긴 `error → done` 이벤트로 종료한다.
  `error.traceId`는 MDC가 리액티브 스레드를 넘지 못하는 특성을 고려해 HTTP 요청
  스레드에서 먼저 캡처한다.
- `helpdesk.chat.stream-timeout`의 `ChatProperties`를 주입해 코드의 60초 상수를 제거했다.
- `ChatStreamControllerTest` 6개로 이벤트 순서·인증 사용자/세션 전달·동시 요청 격리·
  취소 전파·타임아웃·안전 오류를 확인했다.
...
- [x] 채팅 호출 성공·실패·소요시간을 `AI_TOOL_AUDIT`에 기록한다.
- [x] Tool 호출 성공·실패를 누락 없이 기록할 방식을 결정한다.
- [x] 필요하면 별도 AOP Aspect를 추가한다.
- [x] 학번을 마스킹한다.
- [x] 성적·질문 원문·시스템 프롬프트를 로그에 남기지 않는다.
- [x] 예외 메시지에 민감정보가 섞이지 않는지 확인한다.
- [x] 실패 요청도 traceId로 추적할 수 있게 한다.

### 2026-08-22 감사 로그 검증 결과

- `TraceIdFilter`가 모든 HTTP 응답에 서버 생성 `X-Trace-Id`를 추가하고 MDC에 전파
  - 미인증 401 요청도 추적 ID 포함
  - 클라이언트가 보낸 위조 `X-Trace-Id`는 무시하고 서버 값으로 교체
- `AuditAdvisorTest` 2개로 채팅 성공·실패 로그의 마스킹과 예외 메시지 미노출 확인
- `ToolAuditAspectTest` 2개로 Tool 성공·실패, 마스킹 학번, 원문 인자·결과 미노출 확인
- `ToolAuditIntegrationTest`로 Spring AI `MethodToolCallbackProvider`를 통한 실제 Tool 호출도
  감사 Aspect를 거치는지 확인
- 예외은 메시지 대신 `errorType`만 남겨 민감정보 누출을 방지
...
- 최종 리뷰에서 AC-01을 PDF 본칙(21학점·GPA 3.5 이상이면 최대 24학점)에 맞춰 교정하고
  재실행했다. 전체·필수 출처·Tool·근거 없음 모두 100.0%, Tool 오류율 0.0%,
  P95 2,958ms, 호출 오류 0건으로 `goldenSetTest`가 통과했다.
