...
- [ ] 근거 없음 질문이 출처를 만들어내지 않는지 확인한다. — 실측 1/3 통과로 미달
...
- 실행기는 전체 통과율, 출처 적중률, Tool 성공률·오류율, 근거 없음 통과율,
  비스트리밍 P95를 집계한다.
- 2026-08-22 B 실측: 20문항 전체 75.0%, 필수 출처 86.7%, Tool 100.0%,
  근거 없음 33.3%, P95 2,566ms, 호출 오류 0건이다. 품질 기준 미달로
  `goldenSetTest`는 의도대로 실패했다.
